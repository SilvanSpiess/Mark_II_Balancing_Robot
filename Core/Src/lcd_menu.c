/**
  ...
  * @date 28.02.2019
  * @date 23.04.2018
  * @brief  Simple generic lcd menu. Check userCallbackPointer
...
// was in runningMan $todo klj9
		static menuUserDrawItem2 userDrawItemCallback2 = NULL;
		static void* userData;
// end

static void runningMan(uint8_t x, uint8_t y, bool contrastIsInverted)
{
    unsigned updateTreshold = 12;
    static unsigned updateCounter = 0;
    static bool isRunning = false;


    updateCounter++;
    if (updateCounter == updateTreshold) {
        isRunning = !isRunning;
        updateCounter = 0;
    }

    if (isRunning)
        lcd_setSymbol8(x, y, LCD_RUNNING_MAN, contrastIsInverted);
    else
        lcd_setSymbol8(x, y, LCD_STANDING_MAN, contrastIsInverted);
}
